...
#Plot the percentage difference relative to best model for both training and testing of ridge models.

sps.setRidge_testPlot <- ggplot(sps.set_testdf, aes(x = log(lambda), y = percent_diff)) +
  geom_point(aes(size = nzero), alpha = 0.75) +  # Size mapping reintroduced with nzero
  geom_line() +
  geom_point(data = sps.set_testdf[9, ], color = "orange", size = 5, shape = 8, alpha = 0.6) + #Highlight alternative model choice
  geom_text(aes(label = nzero), vjust = -0.8, hjust = 2, color = "darkgrey", size = 3.5) +  # nzero labels reintroduced
...
for (i in 1:length(alphas)){
  ...
  #Append alpha, training MSE, and testing MSE values to the net_elastic dataframe
...
#Index from alphas
best_alpha_index <- which(alphas == best_alpha)

#Extract coefficients from best performing model (lambda min from within best alpha from testing)
best_model_coef <- coef(models[[best_alpha_index]], s = "lambda.min")
...
x_training_sd <- apply(x_training, 2, sd) # Calculate the standard deviations of the covariates.
lasso_min_coef <- as.matrix(coef(lassoCV, s = lassoCV$lambda.min)) # Extract the coefficients and set the object as a matrix.
variables_names <- row.names(lasso_min_coef) # Extract the row names to be added to a column.
lasso_min_coef_df <- as.data.frame(lasso_min_coef, row.names = TRUE) # Change the matrix to a data frame to make it easier to work with.
lasso_min_coef_df$variables <- variables_names # Add the variable names as a new column in the data frame.
lasso_min_coef_df <- lasso_min_coef_df[-1, c(2, 1)] # Remove the intercept and rearrange columns.
lasso_min_coef_df$sd <- x_training_sd # Add the sd's into a new column in the data frame.

ranked_variables <- lasso_min_coef_df %>% #Tidyverse function to process the data frame.
  filter(s1 != 0) %>% # Filter out all the rows that have coefficients the are zero.
  mutate(s1 = abs(s1), scaled_coefs = s1 * sd) %>% # Multiply the absolute values of the coefficients by their respective sd, then add the product to a new column.
  arrange(desc(scaled_coefs)) %>% # Sort the data frame by descending order of the scaled coefficients.
  print() # Print the final data frame to analyze the most influential variables.
